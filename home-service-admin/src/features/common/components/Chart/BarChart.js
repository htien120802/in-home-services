...
import { actionGetQuantityStatisticsByProvider } from 'store/actions';

ChartJS.register(CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend);

function BarChart({dateValue, isYearOnly, id}){
  const dispatch = useDispatch();
  const quantityStatistics = useSelector((state) => state.Admin.quantityStatisticsByProvider)
  ...
  useEffect(() => {
    ...
    if (startDate) {
      ...
      if (isYearOnly) {
        dispatch(actionGetQuantityStatisticsByProvider({ providerId: id, year }));
      } else {
        dispatch(actionGetQuantityStatisticsByProvider({ providerId: id, month, year }));
      }
    }
